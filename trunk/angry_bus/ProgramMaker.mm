...
void ProgramMaker::setVertexShaderFile(const std::string& vs)
{
    _vsFile = vs;
}
void ProgramMaker::setFragmentShaderFile(const std::string& fs)
{
    _fsFile = fs;
}
GLuint ProgramMaker::getProgram()
{
    if (_program == 0)
    {
        buildProgram();
...
bool ProgramMaker::buildProgram()
{
...
GLuint ProgramMaker::buildShader(GLenum st, std::string const &s)
{
